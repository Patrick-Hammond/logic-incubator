...
class RTreeRectangle {
    ...
    AreaIfGrownBy(anotherRect: RTreeRectangle): number {
        if(this.x === Infinity) {
            ...
            return (
                Math.max(this.y + this.height, anotherRect.y + anotherRect.height) -
                Math.min(this.y, anotherRect.y)) *
                (Math.max(this.x + this.width, anotherRect.x + anotherRect.width) - Math.min(this.x, anotherRect.x)) - this.GetArea();
        }
    }
