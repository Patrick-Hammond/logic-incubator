import { Container, Bounds, DRAW_MODES, Texture, BaseTexture, Renderer, Matrix, Rectangle, groupD8 } from "pixi.js";
...
export interface TileOptions {
    ...
    flicker?: number;
    /**
     * Seed that keeps this tile's flicker phase distinct from other flickering tiles. Defaults to a hash of
     * `(x, y)`, which is only stable across frames if `x`/`y` are themselves stable - not true for e.g. a
     * screen-space tile whose position is re-derived from a panning camera every frame. Pass an explicit seed
     * derived from a stable coordinate (a world/grid position) for tiles like that.
     */
    flickerSeed?: number;
}

/** Cheap, deterministic fallback seed so tiles that don't ask for a specific one still decorrelate. */
function defaultFlickerSeed(x: number, y: number): number {
 ...
 * instances.
 *
 * Vendored from `@pixi/tilemap` 3.2.1 (MIT, Ivan Popelyshev) - see `README.md` in this folder for why and what
 * changed.
 */
export class Tilemap extends Container {
    ...
    setTileset(tileset: BaseTexture | Array<BaseTexture> = []): this {
        ...
        for (let i = 0; i < tileset.length; i++) {
            const entry = tileset[i] as unknown as Texture;
            if (entry.baseTexture) {
                tileset[i] = entry.baseTexture;
            }
        }
     ...
     *
     * A trimmed/tightly-packed texture's `tileWidth`/`tileHeight` default to its packed `frame` size (not the
     * untrimmed `orig` size), and its draw position is offset by `texture.trim`, so a trimmed frame lands exactly
     * where a plain `Sprite` would draw it. Upstream (through at least 3.2.1 and the pixi.js-v8-targeting 5.0.2)
     * instead defaults `tileWidth`/`tileHeight` to `texture.orig`, which - for a trimmed, tightly-packed atlas -
     * samples past the real edge of the frame into whatever is packed next to it. See `README.md`.
     *
     ...
     */
    tile(tileTexture: number | string | Texture, x: number, y: number, options: TileOptions = {}): this {
        ...
        if (typeof tileTexture === "number") {
            ...
            for (let i = 0; i < textureList.length; i++) {
                if (textureList[i] === texture.baseTexture) {
                    textureIndex = i;
                    break;
                }
            }
            ...
            options.tileWidth = options.tileWidth ?? texture.frame.width;
            options.tileHeight = options.tileHeight ?? texture.frame.height;
            x += trim ? trim.x : 0;
            y += trim ? trim.y : 0;
    ...
    tileRotate(rotate: number): void {
        ...
        pb[pb.length - (POINT_STRUCT_SIZE - POINT_STRUCT.ROTATE)] = rotate;
    }
    ...
    tileFlicker(intensity: number, seed?: number): void {
        const pb = this.pointsBuf;

        pb[pb.length - (POINT_STRUCT_SIZE - POINT_STRUCT.FLICKER)] = intensity;
        if (seed !== undefined) {
            pb[pb.length - (POINT_STRUCT_SIZE - POINT_STRUCT.FLICKER_SEED)] = seed;
        }
    }
    ...
    renderWebGLCore(renderer: Renderer, plugin: TileRenderer): void {
        ...
        if (this.modificationMarker !== vertices) {
            ...
            for (let i = 0; i < points.length; i += POINT_STRUCT_SIZE) {
                ...
                let u0: number; let v0: number; let u1: number;
