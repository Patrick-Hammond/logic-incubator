...
import {RectTileShader, TilemapShader} from "../shaders/RectTileShader";
    ...
    bindTextures(renderer: PIXI.WebGLRenderer, shader: TilemapShader, textures: Array<PIXI.Texture>) {
        ...
        for(i = 0; i < len; i++) {
            ...
            if(!bs.texture ||
                ...
                if(doClear) {
                    _hackSubImage(
                        (glt.baseTexture as any)._glTextures[renderer.CONTEXT_UID], bs, this._clearBuffer,
                        Constant.boundSize, Constant.boundSize
                    );
                } else {
